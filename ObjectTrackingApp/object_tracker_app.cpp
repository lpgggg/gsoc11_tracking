...
int main(int argc, char** argv)
{
  ...
  const char* directory = "C:/Users/Owner/dev/Tracking_Test_Data/david/imgs";
  const int start = 1;
  const int stop = 462;
  const int delta = 1;
  ...
  cvNamedWindow("Tracker Likelihood");
  for (int frame = start; frame <= stop; frame += delta)
  {
    ...
    // Allocate the likelihood surface
    IplImage* likelihood = cvCreateImage(cvGetSize(image), IPL_DEPTH_8U, 1);
    ...
    {
      // Update the tracker
      if (!tracker.update(image, &theTrack, likelihood))
      {
    ...
    cvShowImage("Tracker Likelihood", likelihood);
  ...
  cvDestroyWindow("Tracker Likelihood");
